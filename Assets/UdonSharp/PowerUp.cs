...
public class PowerUp : UdonSharpBehaviour {
    void Start() {
        Debug.Log("Power up has been spawned");
    }

    private void OnCollisionEnter(Collision collision) {
        Debug.Log("Power up has collided with " + collision.gameObject.name);
        ...
        if (collision.gameObject.GetComponent<PlayerCollider>() != null) {
            ...
            Debug.Log(playerCollider.GetPlayerName() + " has picked up a power up");
        }
